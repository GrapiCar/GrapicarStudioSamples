...
third::third(const grapi::base::String& id, const grapi::base::TypeInfo* type_info) : grapi::runtime::Action(id, type_info) {
}
...
void third::update(const vfloat delta_time) {
    // Update 로직을 여기에 구현하세요
    updateImage(delta_time);    
}


void third::setupImage() {
    // NinePatchImage 설정 로직을 여기에 구현하세요
    grapi::runtime::DisplayObject* target = grapi::runtime::findObjectByName("thirdBox");
...
void third::updateImage(const vfloat delta_time) {
